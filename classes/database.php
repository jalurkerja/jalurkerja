    ...
    class Database {
        ...
        public function fetch_query($sql,$withindex = false){
            ...
            if(mysql_affected_rows($this->db)>0){
                if(mysql_affected_rows($this->db)>1 || $withindex){
                    $arr = array();
                    while($temp = mysql_fetch_array($hsl)){ $arr[] = $temp; }
                    return $arr;
                } else {
                    return mysql_fetch_array($hsl);
                }
            }
            return array();
        }
        ...
        public function insert(){
            ...
            $hsl = $this->execute($sql);
			$arr_return["sql"] = $sql;
			$arr_return["insert_id"] = mysql_insert_id($this->db);
			$arr_return["affected_rows"] = mysql_affected_rows($this->db);
			$arr_return["error"] = mysql_error($this->db);
			$this->fetch_data_clear();
            return $arr_return;
        }
